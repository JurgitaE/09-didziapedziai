const productsList = ['Dviratis', 'Triratis', 'Žvakidė', 'Stalas', 'Kėdė', 'Knygų lentyna', 'Paveikslas su rėmu',
    'Veidrodis', 'Keturių vietų palapinė', 'Pripučiama valtis', 'Meškerė karosams gaudyti', 'Planšetė', 'Pastatoma lempa',
    'Vazonas', 'Baterijų pakrovėjas', 'Pagalvių komplektas', 'Oro drėkintuvas', 'Oro sausintuvas', 'Kavinukas elektrinis',
    'Kilimas 2 X 3 metrų', 'Kilimėlis kojoms valyti', 'Užuolaidos', 'Vėjo malūnas', 'Dažai sienoms', 'Dažai luboms', 'Medinė dėžė'];

// Sukurti 100 sąskaitų masyvą.

/* 
A. Kiekviena sąskaita yra objektas ir turi po 5 savybes:
number: pagal taisykle INV001, INV002, … INV099, INV100;
products: masyvas random ilgio nuo 1 iki 10;
products masyvo elementai objektai, turintys po 4 savybes:
title: random iš products masyvo(vienoje sąskaitoje gali būti keli vienodi produktai.Vistiek jų kainos bus skirtingos);
price: random skaičius su kableliu nuo 1.00 iki 100.00;
amount: random skaičius nuo 1 iki 20;
total:  price ir amount sandauga.
total: visų products masyvo elementų total suma;
vat: 21 % nuo total;
grandTotal: vat ir total suma; 
*/
const rand = (min, max) => Math.floor(Math.random() * (max - min + 1) + min);

const invoices = [...Array(100)].map((_, i) => {
    const number = `INV${'0'.repeat(4 - (String(i + 1).length))}${i + 1}`;
    const products = [...Array(rand(1, 10))].map((_, i) => {
        const title = productsList[rand(0, productsList.length - 1)];
        const price = rand(100, 1000) / 100;
        const amount = rand(1, 20);
        const total = Number((price * amount).toFixed(2));
        return { title, price, amount, total };
    });

    const total = +products.reduce((t, { total }) => t + total, 0).toFixed(2);
    const vat = +(0.21 * total).toFixed(2);
    const grandTotal = +(total + vat).toFixed(2);
    return { number, products, total, vat, grandTotal };

});

console.log(invoices);
/* 
B. Sugeneruotame(ne generavimo metu!) masyve paskaičiuoti ir konsolėje atspausdinti visų sąskaitų grandTotal sumą,
produktų sąrašą prie kiekvieno produkto pavadinimo pridedant koks to produkto kiekis yra visose sąskaitose bendrai ir už
kokią bendrą sumą.
    Pvz: 
Dviratis 35 479.55
Triratis 10 457.22
…..
Medinė dėžė 47 1025.74; 
*/

const soldList = invoices.map(a => a.products).flat();

const list = productsList.forEach((a, i) => {
    const quantity = soldList.reduce((t, c) => a === c.title ? t + c.amount : t, 0);
    const sum = soldList.reduce((t, c) => a === c.title ? t + c.total : t, 0);
    console.log(`${a} ${quantity} ${sum.toFixed(2)}`);
});

const grandTotal = invoices.reduce((t, c) => t + c.grandTotal, 0).toFixed(2);
console.log(grandTotal);